...
#' module_breakout_X201.socioeconomics_Subregions_Malaysia
#'
...
module_breakout_X201.socioeconomics_Subregions_Malaysia <- function(command, ...) {
  if(command == driver.DECLARE_INPUTS) {
    return(c(FILE = "breakout/Subregions_Malaysia_pop",
             FILE = "breakout/Subregions_Malaysia_pcgdp"))
    ...
    city_pop <- get_data(all_data,"breakout/Subregions_Malaysia_pop", strip_attributes = TRUE)
    city_pcgdp <- get_data(all_data,"breakout/Subregions_Malaysia_pcgdp", strip_attributes = TRUE)

    X201.Pop_Subregions_Malaysia <- city_pop %>%
      group_by(region) %>%
    ...
    L201.PCGDP_Subregions_Malaysia <- city_pcgdp %>%
      group_by(region) %>%
      complete(nesting(region), year = c(HISTORICAL_YEARS, FUTURE_YEARS)) %>%
    ...
    # X201.BaseGDP_Subregions_Malaysia:
    X201.BaseGDP_Subregions_Malaysia <- filter(L201.PCGDP_Subregions_Malaysia, year == min(MODEL_BASE_YEARS)) %>%
      left_join_error_no_match(X201.Pop_Subregions_Malaysia, by = c("region", "year")) %>%
      mutate(baseGDP = round(pcgdp * totalPop, socioeconomics.GDP_DIGITS)) %>%
      select(LEVEL2_DATA_NAMES[["BaseGDP"]])

    X201.LaborForceFillout_Subregions_Malaysia <- X201.BaseGDP_Subregions_Malaysia %>%
      mutate(year.fillout = min(MODEL_BASE_YEARS),
             laborforce = socioeconomics.DEFAULT_LABORFORCE) %>%
      select(LEVEL2_DATA_NAMES[["LaborForceFillout"]])

    X201.LaborProductivity_Subregions_Malaysia <- L201.PCGDP_Subregions_Malaysia %>%
      group_by(region) %>%
    ...
    X201.GDP_Subregions_Malaysia <- left_join_error_no_match(X201.Pop_Subregions_Malaysia, L201.PCGDP_Subregions_Malaysia, by = c("region", "year")) %>%
    ...
    X201.Pop_Subregions_Malaysia %>%
      ...
      add_precursors("breakout/Subregions_Malaysia_pop") ->
      X201.Pop_Subregions_Malaysia
    ...
    X201.BaseGDP_Subregions_Malaysia %>%
      ...
      add_precursors("breakout/Subregions_Malaysia_pop","breakout/Subregions_Malaysia_pcgdp") ->
      X201.BaseGDP_Subregions_Malaysia
    ...
    X201.LaborForceFillout_Subregions_Malaysia %>%
      ...
      add_precursors("breakout/Subregions_Malaysia_pcgdp") ->
      X201.LaborForceFillout_Subregions_Malaysia
    ...
    return_data(X201.Pop_Subregions_Malaysia,
                X201.BaseGDP_Subregions_Malaysia,
                X201.LaborForceFillout_Subregions_Malaysia,
                X201.LaborProductivity_Subregions_Malaysia,
                X201.GDP_Subregions_Malaysia)
