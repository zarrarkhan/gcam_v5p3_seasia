...
#' module_breakout_X201.socioeconomics_Bangkok_Thailand
#'
...
module_breakout_X201.socioeconomics_Bangkok_Thailand <- function(command, ...) {
  if(command == driver.DECLARE_INPUTS) {
    return(c(FILE = "breakout/Bangkok_Thailand_pop",
             FILE = "breakout/Bangkok_Thailand_pcgdp"))
  } else if(command == driver.DECLARE_OUTPUTS) {
    return(c("X201.Pop_Bangkok_Thailand",
             "X201.BaseGDP_Bangkok_Thailand",
             "X201.LaborForceFillout_Bangkok_Thailand",
             "X201.LaborProductivity_Bangkok_Thailand",
    ...
    city_pop <- get_data(all_data,"breakout/Bangkok_Thailand_pop", strip_attributes = TRUE)
    city_pcgdp <- get_data(all_data,"breakout/Bangkok_Thailand_pcgdp", strip_attributes = TRUE)

    X201.Pop_Bangkok_Thailand <- city_pop %>%
      group_by(region) %>%
    ...
    L201.PCGDP_Bangkok_Thailand <- city_pcgdp %>%
      group_by(region) %>%
      complete(nesting(region), year = c(HISTORICAL_YEARS, FUTURE_YEARS)) %>%
    ...
    # X201.BaseGDP_Bangkok_Thailand:
    X201.BaseGDP_Bangkok_Thailand <- filter(L201.PCGDP_Bangkok_Thailand, year == min(MODEL_BASE_YEARS)) %>%
      left_join_error_no_match(X201.Pop_Bangkok_Thailand, by = c("region", "year")) %>%
      mutate(baseGDP = round(pcgdp * totalPop, socioeconomics.GDP_DIGITS)) %>%
      select(LEVEL2_DATA_NAMES[["BaseGDP"]])

    X201.LaborForceFillout_Bangkok_Thailand <- X201.BaseGDP_Bangkok_Thailand %>%
      mutate(year.fillout = min(MODEL_BASE_YEARS),
             laborforce = socioeconomics.DEFAULT_LABORFORCE) %>%
      select(LEVEL2_DATA_NAMES[["LaborForceFillout"]])

    X201.LaborProductivity_Bangkok_Thailand <- L201.PCGDP_Bangkok_Thailand %>%
      group_by(region) %>%
    ...
    X201.GDP_Bangkok_Thailand <- left_join_error_no_match(X201.Pop_Bangkok_Thailand, L201.PCGDP_Bangkok_Thailand, by = c("region", "year")) %>%
    ...
    X201.Pop_Bangkok_Thailand %>%
      ...
      add_precursors("breakout/Bangkok_Thailand_pop") ->
      X201.Pop_Bangkok_Thailand
    ...
    X201.BaseGDP_Bangkok_Thailand %>%
      ...
      add_precursors("breakout/Bangkok_Thailand_pop","breakout/Bangkok_Thailand_pcgdp") ->
      X201.BaseGDP_Bangkok_Thailand
    ...
    X201.LaborForceFillout_Bangkok_Thailand %>%
      ...
      add_precursors("breakout/Bangkok_Thailand_pcgdp") ->
      X201.LaborForceFillout_Bangkok_Thailand
    ...
    return_data(X201.Pop_Bangkok_Thailand,
                X201.BaseGDP_Bangkok_Thailand,
                X201.LaborForceFillout_Bangkok_Thailand,
                X201.LaborProductivity_Bangkok_Thailand,
                X201.GDP_Bangkok_Thailand)
