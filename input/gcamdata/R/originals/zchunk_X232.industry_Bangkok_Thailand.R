...
#' module_energy_X232.industry_Bangkok_Thailand
#'
...
module_energy_X232.industry_Bangkok_Thailand <- function(command, ...) {
  if(command == driver.DECLARE_INPUTS) {
    ...
    return(c("X232.DeleteFinalDemand_ind_Bangkok_Thailand",
             ...
             "X232.MAC_higwp_ind_Bangkok_Thailand",
             ...
             "X232.hfc_all_indproc_Bangkok_Thailand",
             "X232.pfc_all_indproc_Bangkok_Thailand"))
    ...
    X201.Pop_Bangkok_Thailand <- get_data(all_data, "X201.Pop_Bangkok_Thailand")
    X201.GDP_Bangkok_Thailand <- get_data(all_data, "X201.GDP_Bangkok_Thailand")
    ...
    X232.DeleteFinalDemand_ind_Bangkok_Thailand <- tibble(region = "Thailand",
                                                  energy.final.demand = unique(L232.PerCapitaBased_ind$energy.final.demand))
    X232.DeleteSupplysector_ind_Bangkok_Thailand <- tibble(region = "Thailand",
                                             supplysector = Industry_sectors)

    # These tables should be split into 2 lists: one for tables with generic info that gets written to Bangkok
    ...
    X232.list_nochange_data_Bangkok_Thailand <- list(
    ...
    X232.list_nochange_data_Bangkok_Thailand <- lapply(X232.list_nochange_data_Bangkok_Thailand,
    ...
    X232.list_nochange_data_Bangkok_Thailand[["L232.StubTechCoef_industry"]]$market.name <-
      X232.list_nochange_data_Bangkok_Thailand[["L232.StubTechCoef_industry"]]$region

    # Industrial process emissions, unlike the fuels, are in local (Bangkok, Rest of Thailand) markets
    X232.list_nochange_data_Bangkok_Thailand[["L231.IndCoef"]] <- X232.list_nochange_data_Bangkok_Thailand[["L231.IndCoef"]] %>%
      rename(stub.technology = technology) %>%
      mutate(market.name = region)

    X232.pop_gdp_share_Bangkok_Thailand <- X201.Pop_Bangkok_Thailand %>%
      left_join_error_no_match(X201.GDP_Bangkok_Thailand, by = c("region", "year")) %>%
      group_by(year) %>%
    ...
    X232.StubTechCalInput_indenergy_Bangkok_Thailand <- L232.StubTechCalInput_indenergy %>%
      downscale_to_breakout_regions(data = .,
                                    composite_region = "Thailand",
                                    disag_regions = c("Bangkok","Rest of Thailand"),
                                    share_data = X232.pop_gdp_share_Bangkok_Thailand, value.column = "calibrated.value", share.column = "gdpshare")

    X232.StubTechCalInput_indfeed_Bangkok_Thailand <- L232.StubTechCalInput_indfeed %>%
      downscale_to_breakout_regions(data = .,
                                    composite_region = "Thailand",
                                    disag_regions = c("Bangkok","Rest of Thailand"),
                                    share_data = X232.pop_gdp_share_Bangkok_Thailand, value.column = "calibrated.value", share.column = "gdpshare")

    X232.StubTechProd_industry_Bangkok_Thailand <- L232.StubTechProd_industry %>%
      downscale_to_breakout_regions(data = .,
                                    composite_region = "Thailand",
                                    disag_regions = c("Bangkok","Rest of Thailand"),
                                    share_data = X232.pop_gdp_share_Bangkok_Thailand, value.column = "calOutputValue", share.column = "gdpshare")

    X232.BaseService_ind_Bangkok_Thailand <- L232.BaseService_ind %>%
      downscale_to_breakout_regions(data = .,
                                    composite_region = "Thailand",
                                    disag_regions = c("Bangkok","Rest of Thailand"),
                                    share_data = X232.pop_gdp_share_Bangkok_Thailand, value.column = "base.service", share.column = "gdpshare")

    X232.pol_emissions_ind_Bangkok_Thailand <- L201.en_pol_emissions %>%
      filter(supplysector %in% Industry_sectors) %>%
      downscale_to_breakout_regions(data = .,
                                    composite_region = "Thailand",
                                    disag_regions = c("Bangkok","Rest of Thailand"),
                                    share_data = X232.pop_gdp_share_Bangkok_Thailand, value.column = "input.emissions", share.column = "gdpshare")

    X232.ghg_emissions_ind_Bangkok_Thailand <- L201.en_ghg_emissions %>%
      filter(supplysector %in% Industry_sectors) %>%
      downscale_to_breakout_regions(data = .,
                                    composite_region = "Thailand",
                                    disag_regions = c("Bangkok","Rest of Thailand"),
    ...
    X232.nonco2_indproc_Bangkok_Thailand <- L232.nonco2_prc %>%
      filter(supplysector %in% Industry_sectors) %>%
      downscale_to_breakout_regions(data = .,
                                    composite_region = "Thailand",
                                    disag_regions = c("Bangkok","Rest of Thailand"),
    ...
    X232.hfc_all_indproc_Bangkok_Thailand <- L241.hfc_all %>%
      downscale_to_breakout_regions(data = .,
                                    composite_region = "Thailand",
                                    disag_regions = c("Bangkok","Rest of Thailand"),
                                    share_data = X232.pop_gdp_share_Bangkok_Thailand, value.column = "input.emissions", share.column = "gdpshare")

    X232.pfc_all_indproc_Bangkok_Thailand <- L241.pfc_all %>%
      downscale_to_breakout_regions(data = .,
                                    composite_region = "Thailand",
                                    disag_regions = c("Bangkok","Rest of Thailand"),
                                    share_data = X232.pop_gdp_share_Bangkok_Thailand, value.column = "input.emissions", share.column = "gdpshare")
    ...
    X232.StubTechMarket_ind_Bangkok_Thailand <- L232.GlobalTechEff_ind %>%
      ...
      repeat_add_columns(tibble(region = c("Bangkok", "Rest of Thailand"))) %>%
      mutate(market.name = "Thailand") %>%
      select(LEVEL2_DATA_NAMES[["StubTechMarket"]])

    X232.StubTechSecMarket_ind_Bangkok_Thailand <- L232.GlobalTechSecOut_ind %>%
      rename(supplysector = sector.name,
             subsector = subsector.name,
             stub.technology = technology) %>%
      repeat_add_columns(tibble(region = c("Bangkok", "Rest of Thailand"))) %>%
    ...
    X232.DeleteFinalDemand_ind_Bangkok_Thailand %>%
    ...
    X232.list_nochange_data_Bangkok_Thailand[["L232.Supplysector_ind"]] %>%
    ...
    X232.list_nochange_data_Bangkok_Thailand[["L232.FinalEnergyKeyword_ind"]] %>%
    ...
    X232.list_nochange_data_Bangkok_Thailand[["L232.SubsectorLogit_ind"]] %>%
    ...
    X232.list_nochange_data_Bangkok_Thailand[["L232.SubsectorShrwtFllt_ind"]] %>%
    ...
    X232.list_nochange_data_Bangkok_Thailand[["L232.SubsectorInterp_ind"]] %>%
        add_title("Subsector shareweight interpolation for industry sector") %>%
        add_units("NA") %>%
    ...
    X232.list_nochange_data_Bangkok_Thailand[["L232.StubTech_ind"]] %>%
    ...
    X232.list_nochange_data_Bangkok_Thailand[["L232.StubTechInterp_ind"]] %>%
      add_title("Identification of stub technologies for industry") %>%
      add_units("NA") %>%
    ...
    X232.list_nochange_data_Bangkok_Thailand[["L232.FuelPrefElast_indenergy"]] %>%
    ...
    X232.list_nochange_data_Bangkok_Thailand[["L232.StubTechCoef_industry"]] %>%
    ...
    X232.StubTechCalInput_indenergy_Bangkok_Thailand %>%
      ...
      add_precursors("L232.StubTechCalInput_indenergy", "X201.Pop_Bangkok_Thailand", "X201.GDP_Bangkok_Thailand") ->
      X232.StubTechCalInput_indenergy_Bangkok_Thailand
    ...
    X232.StubTechCalInput_indfeed_Bangkok_Thailand %>%
      ...
      add_units("calibrated.value: EJ; shareweights: Unitless") %>%
      add_comments("Shareweights are 1 if subsector/technology total is non-zero, 0 otherwise") %>%
      add_precursors("L232.StubTechCalInput_indfeed", "X201.Pop_Bangkok_Thailand", "X201.GDP_Bangkok_Thailand") ->
      X232.StubTechCalInput_indfeed_Bangkok_Thailand
    ...
    X232.StubTechProd_industry_Bangkok_Thailand %>%
      ...
      add_units("calibrated.value: EJ; shareweights: Unitless") %>%
      add_comments("Shareweights are 1 if subsector/technology total is non-zero, 0 otherwise") %>%
      add_precursors("L232.StubTechProd_industry", "X201.Pop_Bangkok_Thailand", "X201.GDP_Bangkok_Thailand") ->
      X232.StubTechProd_industry_Bangkok_Thailand

    X232.list_nochange_data_Bangkok_Thailand[["L232.PerCapitaBased_ind"]] %>%
    ...
    X232.list_nochange_data_Bangkok_Thailand[["L232.IncomeElasticity_ind_gssp2"]] %>%
    ...
    X232.list_nochange_data_Bangkok_Thailand[["L232.PriceElasticity_ind"]] %>%
    ...
    X232.list_nochange_data_Bangkok_Thailand[["L231.UnlimitRsrc"]] %>%
    ...
    X232.list_nochange_data_Bangkok_Thailand[["L231.UnlimitRsrcPrice"]] %>%
    ...
    X232.list_nochange_data_Bangkok_Thailand[["L231.Supplysector_urb_ind"]] %>%
    ...
    X232.list_nochange_data_Bangkok_Thailand[["L231.SubsectorLogit_urb_ind"]] %>%
    ...
    X232.list_nochange_data_Bangkok_Thailand[["L231.SubsectorShrwtFllt_urb_ind"]] %>%
    ...
    X232.list_nochange_data_Bangkok_Thailand[["L231.SubsectorInterp_urb_ind"]] %>%
    ...
    X232.list_nochange_data_Bangkok_Thailand[["L231.StubTech_urb_ind"]] %>%
    ...
    X232.list_nochange_data_Bangkok_Thailand[["L231.IndCoef"]] %>%
    ...
    X232.list_nochange_data_Bangkok_Thailand[["L252.MAC_prc"]] %>%
    ...
    X232.list_nochange_data_Bangkok_Thailand[["L241.fgas_all_units"]] %>%
      add_title("Units of f-gas emissions from industry in Bangkok_Thailand") %>%
      add_units("Specified in table") %>%
      add_comments("Copied") %>%
      add_precursors("common/GCAM_region_names","L241.fgas_all_units") ->
      X232.fgas_all_units_ind_Bangkok_Thailand

    X232.list_nochange_data_Bangkok_Thailand[["L201.nonghg_max_reduction"]] %>%
      add_title("max-reduction of pollutant emissions from industry in Bangkok_Thailand") %>%
      add_units("Percent") %>%
      add_comments("Copied") %>%
      add_precursors("L201.nonghg_max_reduction") ->
      X232.nonghg_max_reduction_ind_Bangkok_Thailand

    X232.list_nochange_data_Bangkok_Thailand[["L201.nonghg_steepness"]] %>%
      add_title("steepness of pollutant emissions from industry in Bangkok_Thailand") %>%
      add_units("Unitless") %>%
      add_comments("Copied") %>%
      add_precursors("L201.nonghg_steepness") ->
      X232.nonghg_steepness_ind_Bangkok_Thailand

    X232.list_nochange_data_Bangkok_Thailand[["L241.hfc_future"]] %>%
      add_title("emissions coefficients of f-gases from industry in Bangkok_Thailand") %>%
      add_units("g f-gas per GJ of building energy service") %>%
      add_comments("Copied") %>%
      add_precursors("common/GCAM_region_names","L241.hfc_future") ->
      X232.hfc_future_ind_Bangkok_Thailand

    X232.list_nochange_data_Bangkok_Thailand[["L252.MAC_higwp"]] %>%
      add_title("MAC curves of f-gases from industry in Bangkok_Thailand") %>%
      add_units("portion of emissions reduced as a fn of the carbon price") %>%
      add_comments("Copied") %>%
      add_precursors("L252.MAC_higwp") ->
      X232.MAC_higwp_ind_Bangkok_Thailand

    X232.list_nochange_data_Bangkok_Thailand[["L232.nonco2_max_reduction"]] %>%
    ...
    X232.list_nochange_data_Bangkok_Thailand[["L232.nonco2_steepness"]] %>%
    ...
    X232.pol_emissions_ind_Bangkok_Thailand %>%
      add_title("pollutant emissions from Bangkok_Thailand industry") %>%
      add_units("Tg/yr") %>%
    ...
    X232.ghg_emissions_ind_Bangkok_Thailand %>%
      add_title("greenhouse gas emissions from Bangkok_Thailand industry") %>%
      add_units("Tg/yr") %>%
      add_comments("downscaled from Thailand using GDP shares") %>%
    ...
    return_data(X232.DeleteFinalDemand_ind_Bangkok_Thailand,
                X232.DeleteSupplysector_ind_Bangkok_Thailand,
                X232.Supplysector_ind_Bangkok_Thailand,
                X232.FinalEnergyKeyword_ind_Bangkok_Thailand,
                X232.SubsectorLogit_ind_Bangkok_Thailand,
